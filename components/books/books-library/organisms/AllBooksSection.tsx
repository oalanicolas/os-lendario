...
export const AllBooksSection: React.FC<AllBooksSectionProps> = ({
  ...
  return (
    <div className="border-t border-border pt-16 md:pt-20">
      <div className="mb-10 flex flex-col gap-4 md:mb-14 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 text-4xl font-bold tracking-tight text-foreground md:text-5xl">
            {totalBookCount}{' '}
            <span className="font-serif font-light italic text-muted-foreground">obras</span>
          </p>
        </div>
        <p className="max-w-xs font-serif text-sm italic text-muted-foreground">
      ...
      {hasMore && !isLoading && <div ref={loadMoreRef} className="mt-16 h-20" />}
      ...
      {!isPageLoading && books.length === 0 && (
        <div className="flex flex-col items-center justify-center py-32 text-center">
          ...
          <p className="mt-2 max-w-sm font-serif text-sm italic text-muted-foreground">
            Nao encontramos livros com os filtros selecionados. Tente ajustar sua busca.
          </p>
          <Button
            variant="outline"
            className="mt-6 h-12 rounded-xl px-8 text-xs font-bold uppercase tracking-wide"
