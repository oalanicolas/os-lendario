    ...
    >
      ...
      <div className="mb-4 flex items-start justify-between">
        {statusBadge ? (
          <Badge
            className={cn(
              'flex items-center gap-1.5 border text-xs font-bold uppercase tracking-wide',
      ...
      <div className="relative mb-6 flex justify-center">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="h-32 w-24 rounded-full bg-primary/0 opacity-0 blur-3xl transition-all duration-700 group-hover:bg-primary/20 group-hover:opacity-100 sm:h-40 sm:w-32" />
        </div>
        ...
        <div className="relative z-10 h-36 w-24 overflow-hidden rounded-lg shadow-xl transition-all duration-500 group-hover:-translate-y-3 group-hover:shadow-[0_25px_50px_rgba(0,0,0,0.6)] sm:h-44 sm:w-28 md:h-52 md:w-36">
          ...
          {/* Subtle spine effect */}
          <div className="absolute bottom-0 left-0 top-0 w-[2px] bg-gradient-to-b from-white/20 via-white/10 to-white/5" />
        </div>
      ...
      <div className="space-y-1.5 text-center">
        ...
        <p className="truncate font-serif text-xs italic text-muted-foreground">{book.author}</p>
      </div>
    </div>
  );
};
