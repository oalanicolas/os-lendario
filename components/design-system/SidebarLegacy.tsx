...
const SidebarLegacy: React.FC<SidebarLegacyProps> = ({
    ...
    // 9. Design System (Hub Completo)
    {
      ...
      children: [
        // Visão Geral
        {
          key: 'ds_overview',
          icon: 'home',
          children: [{ key: 'overview', section: Section.CONCEPT }],
        },
        // Identidade
        {
          key: 'ds_identity',
  ...
  useEffect(() => {
    const isDesignSystemPage =
      Object.values(Section).some((s) => s === currentSection) &&
      navStructure
        .find((n) => n.key === 'design_system_root')
        ?.children?.some(
          (group) =>
            group.children?.some((child) => child.section === currentSection) ||
            group.children?.some((subgroup) =>
              subgroup.children?.some((item) => item.section === currentSection)
            )
        );
  ...
  const renderNavItem = (item: NavItem, depth = 0) => {
    ...
    // Active Logic
    const isSalesAppActive =
      item.key === 'app_sales' && currentSection.startsWith('template_sales');
    const isMindsAppActive = item.key === 'app_minds' && currentSection.startsWith('app_minds');
    const isCreatorAppActive =
      item.key === 'app_creator' && currentSection.startsWith('app_creator');
    const isActive =
      item.section === currentSection || isSalesAppActive || isMindsAppActive || isCreatorAppActive;
          ...
          {isCollapsed ? (
            <div className="flex flex-col items-center gap-4">
              ...
              <Popover
                ...
                trigger={
                  <button
                    ...
                    style={{ backgroundColor: THEMES[currentThemeName].hex }}
                    title="Mudar Tema"
                  />
                }
                content={
                    ...
                    {Object.entries(THEMES).map(([key, themeVal]) => (
                      <button
                        ...
                        className={cn(
                          ...
                          currentThemeName === key && 'ring-2 ring-primary ring-offset-2'
                        )}
              ...
              <Popover
                ...
                align="end"
                trigger={
                  <button className="flex h-6 w-6 items-center justify-center rounded border border-border font-mono text-[10px] font-bold uppercase text-muted-foreground hover:text-foreground">
                    {currentLanguage}
                  </button>
                }
                content={
            ...
            <div className="flex w-full items-center justify-between gap-2">
              ...
              <Popover
                ...
                trigger={
                  <button
                    className="group flex h-9 items-center gap-2 rounded-md border border-transparent p-2 text-muted-foreground transition-colors hover:border-border hover:bg-muted hover:text-foreground"
                    title={t('lang_select')}
                  >
                    ...
                    <Icon
                      name="angle-small-up"
                      className="ml-auto shrink-0 opacity-50 transition-opacity group-hover:opacity-100"
                      size="size-3"
                    />
                  </button>
                }
              ...
              {/* Divider */}
              <div className="h-4 w-px shrink-0 bg-border"></div>
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={toggleTheme}
                  className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-primary"
                ...
                <Popover
                  ...
                  trigger={
                    <button
                      className="group flex h-8 w-8 items-center justify-center rounded-md transition-colors hover:bg-muted"
                      title={t('theme_select')}
                    >
                      <div
                        className="h-4 w-4 shrink-0 rounded-full border border-border/50 shadow-sm transition-transform group-hover:scale-110"
                        style={{ backgroundColor: THEMES[currentThemeName].hex }}
                      />
                    </button>
                  }
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="grid grid-cols-1 gap-1">
                        {Object.entries(THEMES).map(([key, themeVal]) => (
                          ...
                          >
                            <div
                              className="h-3 w-3 shrink-0 rounded-full border border-border/20"
                              style={{ backgroundColor: themeVal.hex }}
                            />
                            {themeVal.label}
                            {currentThemeName === key && (
                              <Icon name="check" className="ml-auto h-3 w-3 text-primary" />
                            )}
                          </button>
                        ))}
                      </div>
