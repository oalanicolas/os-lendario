...
const TiptapEditor: React.FC<TiptapEditorProps> = ({ content, onChange }) => {
  ...
  return (
    <div className="tiptap-wrapper rounded-lg border border-border overflow-hidden bg-card">
      {/* Toolbar */}
      ...
      {mode === 'visual' ? (
        ...
        <textarea
          ...
          className="w-full min-h-[450px] p-6 bg-transparent text-foreground font-mono text-sm leading-relaxed resize-none focus:outline-none"
...
const CourseLesson: React.FC<CourseLessonProps> = ({ setSection }) => {
  ...
  const [editorTab, setEditorTab] = useState<'script' | 'media' | 'exercises' | 'settings'>('script');
    ...
    <div className="flex flex-col h-screen bg-background font-sans overflow-hidden">
      ...
      <div className="h-14 border-b border-border bg-card flex items-center justify-between px-6 shrink-0 z-20">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            className="gap-2 text-[#0A0A0F] font-semibold"
            style={{ backgroundColor: hasChanges ? STUDIO_GOLD : STUDIO_PRIMARY, color: hasChanges ? '#0A0A0F' : 'white' }}
          >
            {isSaving ? (
      ...
      <div className="flex flex-1 overflow-hidden">
                  ...
                  <div className="p-3 space-y-3">
                    {modules.map((mod, modIdx) => (
                      <div key={mod.id} className="space-y-1">
                        <div
                          className="px-2 py-1.5 text-[10px] font-bold uppercase tracking-wider flex justify-between items-center"
                          style={{ color: STUDIO_PRIMARY }}
                        >
                          <span className="truncate">{mod.title}</span>
                          <span className="text-muted-foreground font-mono font-normal text-[10px]">
                            /{mod.lessons.length}
                          </span>
                        </div>
                        {mod.lessons.map((les) => (
                          ...
                          >
                            <span className={cn(
                              ...
                              {les.id === lessonId && <Icon name="check" size="size-2" className="text-white" />}
                            </span>
                            <span className="leading-tight line-clamp-2">{les.title}</span>
                          </Link>
                        ))}
                      </div>
                    ))}
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          {/* TOP TABS BAR */}
          <div className="border-b border-border bg-card/30 px-8 shrink-0">
            ...
            <div className="max-w-4xl mx-auto p-8">
              ...
              {editorTab === 'media' && (
                <div className="space-y-8">
                  <div>
                    <h2 className="text-xl font-semibold mb-1">Mídia & Materiais</h2>
                    <p className="text-sm text-muted-foreground">Adicione vídeos e materiais de apoio para esta aula.</p>
                  </div>
                  ...
                  <Card>
                    <CardContent className="p-6 space-y-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-[#538096]/10 flex items-center justify-center">
                      ...
                      {videoUrl && (
                        <div className="aspect-video bg-muted rounded-lg flex items-center justify-center border border-border">
                          <div className="text-center">
                            <Icon name="play-circle" size="size-12" className="text-muted-foreground mb-2" />
              ...
              {editorTab === 'exercises' && (
                <div className="space-y-8">
                  <div>
                    <h2 className="text-xl font-semibold mb-1">Exercícios & Avaliações</h2>
                    <p className="text-sm text-muted-foreground">Crie quizzes, assessments e atividades práticas para esta aula.</p>
                  </div>
                  ...
                  <Card>
                    <CardContent className="p-6 space-y-4">
                      ...
                      <div className="border border-dashed border-border rounded-lg p-6 text-center">
                        <Icon name="circle-question" size="size-8" className="mx-auto mb-3 text-muted-foreground/50" />
                        <p className="text-sm text-muted-foreground mb-2">Nenhum quiz criado</p>
                        <p className="text-xs text-muted-foreground">Quizzes ajudam a fixar o conteúdo e medir compreensão</p>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-6 space-y-4">
                      ...
                      <div className="border border-dashed border-border rounded-lg p-6 text-center">
                        <Icon name="clipboard-check" size="size-8" className="mx-auto mb-3 text-muted-foreground/50" />
                        <p className="text-sm text-muted-foreground mb-2">Nenhum assessment criado</p>
                        <p className="text-xs text-muted-foreground">Assessments avaliam o domínio completo do conteúdo</p>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-6 space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">Atividade hands-on para aplicar o conhecimento</p>
                          </div>
                        </div>
                        <Button variant="outline" size="sm" className="gap-2">
                          <Icon name="plus" size="size-4" />
                      ...
                      <div className="border border-dashed border-border rounded-lg p-6 text-center">
                        <Icon name="laptop-code" size="size-8" className="mx-auto mb-3 text-muted-foreground/50" />
                        <p className="text-sm text-muted-foreground mb-2">Nenhum exercício prático</p>
                        <p className="text-xs text-muted-foreground">Exercícios práticos consolidam o aprendizado através da ação</p>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardContent className="p-6 space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">Questão aberta para reflexão do aluno</p>
                          </div>
                        </div>
                        <Button variant="outline" size="sm" className="gap-2">
                          <Icon name="plus" size="size-4" />
                      ...
                      <div className="border border-dashed border-border rounded-lg p-6 text-center">
                        <Icon name="lightbulb" size="size-8" className="mx-auto mb-3 text-muted-foreground/50" />
                        <p className="text-sm text-muted-foreground mb-2">Nenhuma pergunta de reflexão</p>
                        <p className="text-xs text-muted-foreground">Perguntas abertas estimulam pensamento crítico</p>
                      </div>
                    </CardContent>
                  </Card>
                </div>
              ...
              {editorTab === 'settings' && (
                <div className="space-y-8">
                  <div>
                    <h2 className="text-xl font-semibold mb-1">Configurações da Aula</h2>
                    ...
                    <CardContent className="p-6 space-y-6">
                      <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          />
                          <p className="text-xs text-muted-foreground">Tempo aproximado para completar a aula</p>
                        </div>
                        <div className="space-y-2">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div className="p-3 bg-muted/30 rounded-lg">
                            ...
                            <p className="font-mono text-xs mt-1">{lesson.id}</p>
                          </div>
                          <div className="p-3 bg-muted/30 rounded-lg">
                            <span className="text-muted-foreground">Slug</span>
                            <p className="font-mono text-xs mt-1">{lesson.slug}</p>
                          </div>
                          <div className="p-3 bg-muted/30 rounded-lg">
                            <span className="text-muted-foreground">Status</span>
                            <p className="text-xs mt-1">{lesson.status}</p>
                          </div>
                          <div className="p-3 bg-muted/30 rounded-lg">
                            <span className="text-muted-foreground">Fidelidade</span>
                            <p className="text-xs mt-1">{lesson.fidelity_score ? `${Math.round(lesson.fidelity_score * 100)}%` : 'N/A'}</p>
                          </div>
                        </div>
                      </div>
                    </CardContent>
