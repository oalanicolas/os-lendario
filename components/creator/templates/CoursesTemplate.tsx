...
import { useRecentActivities, formatRelativeTime, getIconForTipoLabel } from '../../../hooks/useRecentActivities';
...
const coursesMock: Course[] = [
    // 1. Claude Code Expert
    {
        ...
        instructor: { name: 'José Carlos Amorim', avatar: 'https://i.pravatar.cc/150?u=jose', isMMOS: true },
        lessonsCount: 11,
        modulesCount: 3,
        researchCount: 5,
        assessmentsCount: 3,
        ...
        pipeline: { brief: 'completed', research: 'completed', curriculum: 'completed', lessons: 'completed', validation: 'completed' }
    },
    // 2. Didática Lendária
    {
        id: 'course_002',
        ...
        instructor: { name: 'Adriano de Marqui', avatar: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?q=80&w=200&auto=format&fit=crop', isMMOS: true },
        lessonsCount: 35,
        modulesCount: 7,
        researchCount: 12,
        assessmentsCount: 7,
        ...
        pipeline: { brief: 'completed', research: 'completed', curriculum: 'completed', lessons: 'current', validation: 'pending' }
    },
    // 3. Vibecoding
    {
        id: 'course_003',
        ...
        instructor: { name: 'Alan Nicolas', avatar: 'https://yt3.googleusercontent.com/JNCtRSKK2aMQOmEroyBWmdbiaq_uTlFn-h_RuhGakkxBHW14e9u4yMZk884Espvk8he9GIYjrPE=s900-c-k-c0x00ffffff-no-rj', isMMOS: false },
        lessonsCount: 7,
        modulesCount: 3,
        researchCount: 3,
        assessmentsCount: 2,
    ...
    // 5. Dominando Obsidian
    {
        ...
        pipeline: { brief: 'completed', research: 'completed', curriculum: 'completed', lessons: 'completed', validation: 'current' }
    },
    // 6. Método M.A.P.A.
    {
        id: 'course_006',
...
const generationLog = [
    ...
    { id: "1.3", title: "Diagnóstico", gps: 45, dl: 82, status: "retrying", msg: "GPS: Destino pouco claro. Regenerando..." },
...
const CoursesTemplate: React.FC<{ setSection: (s: Section) => void }> = ({ setSection }) => {
    ...
    const { courses: hookCourses, loading: isLoading, error, isUsingMockData, refetch } = useCourses();
    ...
    const PipelineStep = ({ status, label }: { status: 'completed' | 'current' | 'pending', label: string }) => {
        let iconName = "circle";
        ...
        return (
            <div className="flex flex-col items-center gap-1 group cursor-help relative">
                ...
                <span className={cn("text-[9px] uppercase tracking-wider transition-colors", labelClass)}>{label}</span>
            </div>
        );
    };
    ...
    const PipelineVisual = ({ pipeline }: { pipeline: Course['pipeline'] }) => (
        <div className="flex items-center justify-between w-full relative">
            ...
            <div className="bg-card px-1 z-10"><PipelineStep status={pipeline.research} label="Research" /></div>
            <div className="bg-card px-1 z-10"><PipelineStep status={pipeline.curriculum} label="Currículo" /></div>
            <div className="bg-card px-1 z-10"><PipelineStep status={pipeline.lessons} label="Lições" /></div>
            <div className="bg-card px-1 z-10"><PipelineStep status={pipeline.validation} label="Validação" /></div>
        </div>
    );
        ...
        <div className="flex items-center justify-between mb-8 animate-fade-in">
            <div>
                <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
                    <span className="cursor-pointer hover:text-foreground" onClick={() => setView('list')}>Cursos</span>
                    {breadcrumb && (
                        <>
                ...
                <h1 className="text-3xl font-sans font-bold text-foreground tracking-tight">{title}</h1>
            </div>
            <div className="flex gap-3">
                ...
                {view === 'list' && (
                    <Button onClick={() => setView('new')} className="bg-primary text-primary-foreground shadow-lg shadow-primary/20 hover:scale-105 transition-transform">
    ...
    const renderList = () => {
        ...
        const avgFidelity = courses.filter(c => c.fidelityScore !== null).length > 0
            ? Math.round(courses.filter(c => c.fidelityScore !== null).reduce((acc, c) => acc + (c.fidelityScore || 0), 0) / courses.filter(c => c.fidelityScore !== null).length)
            : null;
        ...
        const lowFidelityCourses = courses.filter(c => c.fidelityScore !== null && c.fidelityScore < 80);
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    { label: 'Cursos Ativos', value: courses.length, icon: 'graduation-cap', change: '+12%', changeLabel: 'vs mês anterior', trendUp: true, sparkline: '0,20 10,15 20,25 30,18 40,22 50,10 60,15 70,5 80,10 90,0' },
                    { label: 'Total de Lições', value: totalLessons, icon: 'document', change: '+5', changeLabel: 'novas esta semana', trendUp: true, sparkline: '0,25 10,22 20,20 30,15 40,18 50,12 60,10 70,8 80,5 90,2' },
                    { label: 'Horas de Conteúdo', value: '28.8h', icon: 'clock', change: '—', changeLabel: '0% atualizado hoje', trendUp: false, sparkline: '0,15 10,15 20,15 30,15 40,15 50,15 60,15 70,15 80,15 90,15' },
                    { label: 'Alunos Impactados', value: '3.2k', icon: 'users', change: '+15%', changeLabel: 'vs mês anterior', trendUp: true, sparkline: '0,28 10,25 20,22 30,20 40,15 50,10 60,12 70,8 80,5 90,0' },
                ].map((metric, idx) => (
                    <Card key={idx} className="bg-card border-border hover:border-[#538096]/50 transition-all group relative overflow-hidden">
                        ...
                        <CardContent className="p-5 flex items-start justify-between relative z-10">
                            <div>
                                ...
                                <div className="flex items-center gap-1 mt-2">
                                    ...
                                    <span className={cn("text-[10px] font-bold", metric.trendUp ? "text-emerald-600" : "text-muted-foreground")}>
                                        {metric.change}
                                    </span>
                                    <span className="text-[10px] text-muted-foreground ml-1">{metric.changeLabel}</span>
                                </div>
                            </div>
                            <div
            ...
            <Card className="bg-card border-border hover:border-[#538096]/50 transition-colors cursor-pointer group">
                <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <Badge variant="outline" className="border-border text-muted-foreground group-hover:text-[#538096] group-hover:border-[#538096] transition-colors">
                    ...
                    <div className="flex items-center justify-between relative min-w-[700px]">
                        ...
                        <div className="absolute top-5 left-0 w-full h-0.5 bg-muted -z-0">
                            <div className="h-full w-[15%] transition-all duration-1000" style={{ backgroundColor: STUDIO_PRIMARY }} />
                        </div>

                        {[
                            { id: 'briefing', label: 'BRIEFING', icon: 'file-edit', count: pipelineCounts.briefing || 8, status: 'active' },
                            { id: 'research', label: 'PESQUISA', icon: 'search', count: pipelineCounts.research || 3, status: 'pending' },
                            { id: 'curriculum', label: 'CURRÍCULO', icon: 'list', count: pipelineCounts.curriculum || 1, status: 'pending' },
                            { id: 'generation', label: 'GERAÇÃO', icon: 'magic-wand', count: pipelineCounts.generation || 2, status: 'pending' },
                            { id: 'validation', label: 'VALIDAÇÃO', icon: 'check-circle', count: pipelineCounts.validation || 0, status: 'pending' },
                            { id: 'production', label: 'PRODUÇÃO', icon: 'video-camera', count: 4, status: 'pending' },
                            { id: 'published', label: 'PUBLICADO', icon: 'rocket', count: pipelineCounts.published || 12, status: 'done' },
                        ].map((stage) => (
                            <div key={stage.id} className="flex flex-col items-center gap-3 relative z-10 group/step">
                                <div
                                    ...
                                    style={{
                                        ...
                                        borderColor: stage.status === 'active' || stage.status === 'done' ? STUDIO_PRIMARY : undefined,
                                        boxShadow: stage.status === 'active' ? `0 0 20px ${STUDIO_PRIMARY}40` : undefined
                                    }}
                                >
                                    {stage.status === 'done' ? <Icon name="check" size="size-4" /> : <Icon name={stage.icon} size="size-4" />}
                                </div>
                                <div className="text-center">
                                    <p
                                        className="text-xs font-bold uppercase tracking-wider mb-0.5"
                                        style={stage.status === 'active' || stage.status === 'done' ? { color: STUDIO_PRIMARY } : {}}
                                    >
                                        {stage.label}
                                    </p>
                                    <p className="text-sm font-mono text-muted-foreground">{stage.count}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <Card className="bg-card/50 border-border/30">
                        <CardContent className="p-3 flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                            <div className="relative flex-1">
                                ...
                                <Input placeholder="Buscar cursos..." className="pl-10 h-10 bg-muted/20 border-border/30 rounded-lg" />
                            </div>
                            <Select
                                placeholder="Todos"
                                options={[
                                    ...
                                    <div className={cn(
                                        ...
                                        <div className={cn("flex-1 min-w-0", viewMode === 'grid' && "w-full")}>
                                            ...
                                            <div className={cn("flex gap-2 mb-1.5", viewMode === 'grid' ? "items-start justify-between" : "items-center")}>
                                                ...
                                                {viewMode === 'grid' && (
                                                    <Button variant="ghost" size="icon" className="h-7 w-7 -mt-1 -mr-2 text-muted-foreground hover:text-foreground shrink-0" onClick={(e) => e.stopPropagation()}>
                                            ...
                                            <div className={cn(
                                                ...
                                                viewMode === 'grid'
                                                    ? "flex-col gap-2 border-t border-border pt-3 mt-3"
                                                    : "items-center gap-4"
                                            )}>
                                                <span className="flex items-center gap-1.5 font-medium" style={{ color: STUDIO_GOLD }}>
                                        ...
                                        <div className={cn(
                                            ...
                                            viewMode === 'grid'
                                                ? "w-full items-center justify-between border-t border-border pt-3 mt-auto"
                                                : "hidden md:flex flex-col items-end"
                                        )}>
                                            {course.progress === 100 ? (
                                                <Badge variant="outline" className="border-emerald-500/30 text-emerald-600 bg-emerald-500/10 text-[10px] uppercase tracking-wider px-2 py-0.5">
                                                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-1.5 animate-pulse" />
                                                    Completo
                                                </Badge>
                                            ) : (
                                                <div className={cn("space-y-1", viewMode === 'grid' ? "flex-1 mr-4" : "w-28")}>
                    ...
                    <Card className="bg-card border-border shadow-sm">
                        ...
                        <CardContent className="pt-4 px-4 pb-0">
                            <div className="space-y-0 relative">
                                ...
                                <div className="absolute left-4 top-2 bottom-6 w-px bg-border" />

                                {activitiesLoading ? (
                                    // Loading skeleton
                                    [1, 2, 3].map((i) => (
                                        <div key={i} className="flex gap-4 relative pb-6">
                                            <div className="w-8 h-8 rounded-lg bg-muted/30 animate-pulse z-10 shrink-0" />
                                    ...
                                    <div className="text-center py-4">
                                        <Icon name="inbox" size="size-6" className="mx-auto text-muted-foreground/40 mb-2" />
                                    ...
                                    recentActivities.map((activity, i) => (
                                        <div
                                            key={activity.id}
                                            className="flex gap-4 relative pb-6 last:pb-4 group cursor-pointer"
                                            onClick={() => navigate(`/creator/cursos/${activity.project_slug}`)}
                                        >
                                            <div
                                                className="w-8 h-8 rounded-lg flex items-center justify-center shadow-sm z-10 shrink-0 border border-white/10"
                                                style={{ backgroundColor: STUDIO_ACCENT, color: STUDIO_PRIMARY }}
                                            >
                                            ...
                                            <div className="flex-1 min-w-0 pt-0.5">
                                                ...
                                                <p className="text-sm font-medium text-foreground truncate" title={activity.title}>
                                                    {activity.title}
                                                </p>
                            ...
                            {[
                                ...
                                <div key={i} className="space-y-1.5">
                                    <div className="flex justify-between text-xs">
                                        ...
                                        <span className="text-muted-foreground font-mono">{type.count}</span>
                                    </div>
            ...
            <div className="max-w-4xl mx-auto space-y-12 animate-fade-in pb-20">
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Card 
                            className={cn("cursor-pointer transition-all duration-300 hover:shadow-lg border-2", mode === 'greenfield' ? "border-primary bg-primary/5" : "border-border hover:border-primary/50")}
                            onClick={() => setMode('greenfield')}
                        >
                            <CardContent className="p-8 space-y-6">
                                <div className="flex items-center justify-between">
                                    <div className="w-12 h-12 rounded-full bg-brand-green/10 flex items-center justify-center text-brand-green"><Icon name="seedling" size="size-6" /></div>
                                    {mode === 'greenfield' && <Icon name="check-circle" className="text-primary size-6" type="solid" />}
                                </div>
                                <div><h4 className="text-lg font-bold">Greenfield (Do Zero)</h4><p className="text-xs text-muted-foreground font-mono uppercase tracking-wider">Recomendado</p></div>
                                <p className="text-sm text-muted-foreground font-serif leading-relaxed">Crie uma estrutura pedagógica perfeita (GPS + Didática Lendária) sem vícios antigos.</p>
                            </CardContent>
                        </Card>
                    </div>
                </div>
                        ...
                        <CardContent className="p-6 space-y-6">
                            <div className="grid gap-6 md:grid-cols-2">
                                <div className="space-y-2">
                                    ...
                                    <Input placeholder="ex: vendas-b2b-avancado" value={slug} onChange={handleSlugChange} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-sm font-medium">Instrutor / Persona</label>
                                    <Select placeholder="Selecione um instrutor..." value={persona} onValueChange={setPersona} options={[{ label: "MMOS: Alan Nicolas", value: "an" }]} />
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                ...
                <div className="flex justify-end gap-4 pt-8 border-t border-border">
                    ...
                    <Button onClick={handleCreateCourse} disabled={!mode || !slug} className="min-w-[140px]">
                        {isCreating ? <Icon name="spinner" className="mr-2 animate-spin" /> : <>Criar Curso <Icon name="arrow-right" className="ml-2" /></>}
                    </Button>
                </div>
            </div>
        </>
    ...
    const renderBriefEditor = () => (
        <div className="animate-fade-in pb-20">
            <CoursesHeader title="Briefing Estratégico" breadcrumb={`Cursos > ${slug || 'novo'} > Brief`} />
            <div className="flex flex-col lg:flex-row gap-8">
                <div className="w-full lg:w-64 shrink-0 space-y-6">
                    <Card className="border-border bg-card">
                        <CardHeader className="pb-2"><CardTitle className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Seções</CardTitle></CardHeader>
                        <CardContent className="p-2">
                            <nav className="space-y-1">
                                {[1,2,3,4,5,6,7,8].map((id) => (
                                    <button key={id} onClick={() => setActiveSection(id)} className={cn("w-full flex items-center justify-between px-3 py-2 rounded-md text-sm transition-colors text-left", activeSection === id ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-muted")}>
                                        <span className="truncate">Seção {id}</span>
                                        {activeSection > id && <Icon name="check-circle" className="text-success size-3" type="solid" />}
                                    </button>
                                ))}
                            </nav>
                        </CardContent>
                ...
                <div className="flex-1 space-y-6">
                    <Card>
                        <CardContent className="p-6 space-y-6">
                            <div className="space-y-2">
                                ...
                                <Textarea placeholder="Ex: Dominar vendas B2B em 30 dias..." className="min-h-[100px]" />
                            </div>
                            <div className="space-y-2">
                    ...
                    <div className="flex justify-between pt-6 border-t border-border">
                        <Button variant="outline" disabled={activeSection === 1} onClick={() => setActiveSection(p => p - 1)}>Anterior</Button>
                        {activeSection < 8 ? (
                            <Button onClick={() => setActiveSection(p => p + 1)}>Próxima Seção</Button>
                        ) : (
                            <Button onClick={startResearch} className="shadow-lg shadow-primary/20">Iniciar Pesquisa de Mercado <Icon name="search-alt" className="ml-2" /></Button>
                        )}
                    </div>
                </div>
            </div>
    ...
    const renderResearchLoading = () => (
        <div className="flex flex-col items-center justify-center min-h-[60vh] animate-fade-in text-center space-y-8">
            <div className="relative w-32 h-32">
                ...
                <Icon name="search-alt" className="absolute inset-0 m-auto text-primary size-10 animate-pulse" />
            </div>
            <div className="space-y-2">
                <h3 className="text-2xl font-bold font-sans">Agente de Pesquisa Ativo</h3>
                <p className="text-muted-foreground font-serif">Analisando concorrentes, identificando gaps e buscando diferenciais...</p>
                <div className="text-xs font-mono text-muted-foreground pt-4 space-y-1">
    ...
    const renderResearchResults = () => (
        <div className="animate-fade-in space-y-8 pb-20">
            <CoursesHeader title="Inteligência de Mercado" breadcrumb={`Cursos > ${slug || 'novo'} > Pesquisa`} />
            
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <Card className="lg:col-span-2">
                    <CardHeader><CardTitle>Análise de Concorrência</CardTitle></CardHeader>
                    <CardContent className="space-y-4">
                        <Alert>
                            <Icon name="info" className="size-4" />
                            <AlertTitle>Insight Principal</AlertTitle>
                            <AlertDescription>90% dos cursos focam em teoria. A maior reclamação é falta de templates práticos.</AlertDescription>
                        </Alert>
                ...
                <Card className="bg-primary/5 border-primary/20">
                    <CardHeader><CardTitle className="text-primary">Oportunidades (Gaps)</CardTitle></CardHeader>
                    <CardContent className="space-y-3">
                        <div className="flex gap-2 items-start"><Icon name="check-circle" className="text-primary mt-1 size-4"/><span className="text-sm">Incluir módulo sobre IA aplicada</span></div>
                        <div className="flex gap-2 items-start"><Icon name="check-circle" className="text-primary mt-1 size-4"/><span className="text-sm">Fornecer scripts prontos (Copy & Paste)</span></div>
                        <div className="flex gap-2 items-start"><Icon name="check-circle" className="text-primary mt-1 size-4"/><span className="text-sm">Focar em Mobile-First learning</span></div>
                    </CardContent>
                </Card>
            </div>
    ...
    const renderReformulation = () => (
        <div className="animate-fade-in space-y-8 pb-20">
            <CoursesHeader title="Reformulação do Brief" breadcrumb={`Cursos > ${slug || 'novo'} > Diff`} />
            
            <div className="grid grid-cols-2 gap-8">
                <div className="space-y-4">
                    <h3 className="font-bold text-muted-foreground uppercase tracking-widest text-sm text-center">Seu Brief Original</h3>
                    <Card className="opacity-70 border-dashed">
                        <CardContent className="p-6 text-sm font-serif">
                            <p className="mb-2"><strong>Promessa:</strong> Ensinar vendas B2B.</p>
                            <p><strong>Público:</strong> Vendedores.</p>
                            <p><strong>Diferencial:</strong> Minha experiência.</p>
                        </CardContent>
                    </Card>
                </div>
                <div className="space-y-4">
                    <h3 className="font-bold text-primary uppercase tracking-widest text-sm text-center">Sugestão da IA (Baseada em Dados)</h3>
                    <Card className="border-primary bg-primary/5 shadow-lg relative">
                        <Badge className="absolute -top-3 right-4 bg-primary text-primary-foreground">Otimizado</Badge>
                        <CardContent className="p-6 text-sm font-serif">
                            <p className="mb-2"><strong>Promessa:</strong> <span className="bg-primary/20 px-1 rounded">Dominar o ciclo de vendas B2B e fechar contratos High-Ticket em 30 dias.</span></p>
                            <p><strong>Público:</strong> <span className="bg-primary/20 px-1 rounded">Account Executives e SDRs buscando promoção.</span></p>
                            <p><strong>Diferencial:</strong> <span className="bg-primary/20 px-1 rounded">Único com templates de IA e foco em Social Selling.</span></p>
                        </CardContent>
                    </Card>
                </div>
            </div>
                        ...
                        {[1, 2, 3].map((m) => (
                            <Card key={m}>
                                ...
                                <CardContent className="p-0">
                                    {[1, 2, 3].map((l) => (
                                        <div key={l} className="p-4 border-b border-border last:border-0 flex justify-between items-center">
        ...
        return (
            <div className="animate-fade-in space-y-8 pb-20">
                ...
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex gap-2">
                        <Button variant="outline" size="sm"><Icon name="refresh" className="mr-2" /> Regenerar</Button>
                        <Button size="sm" onClick={startGeneration} className="shadow-lg shadow-primary/20">Aprovar e Criar Aulas</Button>
                    </div>
                </div>

                {modules.length === 0 ? (
                    <Card className="p-12 text-center">
                        <Icon name="folder-open" className="mx-auto text-muted-foreground mb-4" size="size-12" />
                        <h3 className="text-lg font-bold mb-2">Nenhum conteúdo encontrado</h3>
                        <p className="text-muted-foreground text-sm mb-4">Este curso ainda não tem módulos ou lições cadastrados.</p>
                            ...
                            <Card key={mod.id}>
                                <CardHeader className="py-3 bg-muted/20 border-b border-border">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <Badge variant="outline" className="text-[10px]">{mod.lessons.length} aulas</Badge>
                                    </div>
                                    {mod.description && (
                                ...
                                <CardContent className="p-0">
                                    {mod.lessons.map((les, lesIndex) => (
                                        <div key={les.id} className="p-4 border-b border-border last:border-0 flex justify-between items-center hover:bg-muted/10 group cursor-pointer"
                                            onClick={() => { setSlug(les.slug); setView('lesson'); }}
                                        >
                                            <div className="flex items-center gap-3">
                                                <Icon name="menu-burger" className="text-muted-foreground opacity-30 group-hover:opacity-100 cursor-grab" size="size-3" />
                                            ...
                                            <div className="flex items-center gap-4">
                                                {les.fidelity_score && (
                                                    <Badge variant={les.fidelity_score >= 80 ? 'success' : les.fidelity_score >= 60 ? 'warning' : 'destructive'} className="text-[10px]">
                                                ...
                                                <Badge variant={les.status === 'published' ? 'success' : 'outline'} className="text-[10px]">
                                                    {les.status === 'published' ? 'Publicado' : les.status === 'draft' ? 'Rascunho' : les.status}
                                                </Badge>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-4">
                    {generationLog.map((log) => (
                        <Card key={log.id} className={cn("border-l-4 transition-all", 
                            log.status === 'success' ? "border-l-success bg-success/5" : 
                            log.status === 'retrying' ? "border-l-brand-yellow bg-brand-yellow/5" : "border-l-muted bg-muted/5 opacity-60"
                        )}>
                            <div className="p-4 flex justify-between items-center">
                                <div className="flex items-center gap-4">
                                    {log.status === 'success' ? <Icon name="check-circle" className="text-success" /> : 
                                     log.status === 'retrying' ? <Icon name="refresh" className="text-brand-yellow animate-spin" /> : 
                                     <Icon name="circle" className="text-muted-foreground" />}
                                    
                                    <div>
                                        <p className="font-bold text-sm">{log.id} - {log.title}</p>
                                        <p className="text-xs text-muted-foreground">{log.msg}</p>
                                    </div>
                                </div>
                                <div className="flex gap-4 text-xs font-mono">
                                    <div className="text-center">
                                        <span className="block text-[10px] text-muted-foreground uppercase">GPS Score</span>
                                        <span className={cn("font-bold", log.gps > 70 ? "text-success" : log.gps > 0 ? "text-brand-yellow" : "text-muted")}>{log.gps > 0 ? log.gps : '-'}</span>
                                    </div>
                                    <div className="text-center">
                                        <span className="block text-[10px] text-muted-foreground uppercase">DL Score</span>
                                        <span className={cn("font-bold", log.dl > 70 ? "text-success" : log.dl > 0 ? "text-brand-yellow" : "text-muted")}>{log.dl > 0 ? log.dl : '-'}</span>
                                    </div>
                                </div>
                            </div>
                        </Card>
                ...
                <Card className="h-fit">
                    ...
                    <CardContent className="space-y-6">
                        <div className="text-center">
                            <div className="text-4xl font-bold font-mono text-primary mb-2">35%</div>
                            <Progress value={35} className="h-2" />
                            <p className="text-xs text-muted-foreground mt-2">Estimativa: 4 min restantes</p>
                        </div>
                        <Separator />
                        <div className="space-y-2">
                            <h4 className="text-xs font-bold uppercase tracking-wider">Quality Gate (Validação)</h4>
                            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Framework GPS</span> <span className="text-success font-bold">Pass</span></div>
                            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Didática Lendária</span> <span className="text-success font-bold">Pass</span></div>
                            <div className="flex justify-between text-sm"><span className="text-muted-foreground">Voice Consistency</span> <span className="text-brand-yellow font-bold">Checking...</span></div>
                        </div>
                        <Button className="w-full" variant="outline" onClick={goToLesson}>Simular Finalização</Button>
                    </CardContent>
                </Card>
            </div>
        </div>
    ...
    const renderValidation = () => (
        <div className="flex flex-col items-center justify-center min-h-[60vh] animate-fade-in text-center space-y-8">
            ...
            <h1 className="text-4xl font-bold font-sans">Curso Pronto para Publicação!</h1>
            <p className="text-xl text-muted-foreground font-serif max-w-2xl">
                Todas as lições foram geradas, validadas pelos frameworks GPS e Didática Lendária, e aprovadas.
            </p>
            <div className="grid grid-cols-3 gap-8 text-left bg-card border border-border p-8 rounded-xl shadow-lg">
                <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-widest font-bold">Aulas</p>
                    <p className="text-3xl font-bold mt-1">12</p>
                </div>
                <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-widest font-bold">Duração</p>
                    <p className="text-3xl font-bold mt-1">2h 15m</p>
                </div>
                <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-widest font-bold">Materiais</p>
                    <p className="text-3xl font-bold mt-1">5 PDFs</p>
                </div>
            </div>
            <div className="flex gap-4 pt-4">
                <Button variant="outline" size="lg" onClick={goBack}>Voltar ao Dashboard</Button>
                <Button size="lg" className="bg-primary text-primary-foreground shadow-xl">Publicar Agora</Button>
            </div>
        </div>
    );
    ...
    return (
        <div className="flex flex-col min-h-screen bg-background font-sans pb-20">
            ...
            {view !== 'lesson' && <CreatorTopbar currentSection={Section.APP_CREATOR_COURSES} setSection={setSection} />}
            
            <main className={cn("w-full mx-auto", view !== 'lesson' ? "p-6 md:p-12 max-w-[1400px]" : "p-0")}>
