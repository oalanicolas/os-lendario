...
const SheetContent: React.FC<SheetContentProps> = ({
  ...
  return (
    <div
      className={cn(
        'fixed inset-y-0 z-50 flex h-full w-3/4 flex-col border-border bg-card p-6 shadow-2xl transition-transform duration-300 ease-in-out sm:max-w-sm',
...
const SheetHeader: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ className, ...props }) => (
  <div
    className={cn('mb-6 flex flex-col space-y-2 text-center sm:text-left', className)}
    {...props}
  />
);
...
const SheetFooter: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ className, ...props }) => (
  <div
    className={cn(
      'mt-auto flex flex-col-reverse border-t border-border pt-6 sm:flex-row sm:justify-end sm:space-x-2',
      className
    )}
    {...props}
  />
...
const SheetTitle: React.FC<React.HTMLAttributes<HTMLHeadingElement>> = ({
  ...
  <h2 className={cn('font-sans text-lg font-semibold text-foreground', className)} {...props} />
);
