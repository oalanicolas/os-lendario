...
const alertVariants = cva(
  'relative w-full rounded-lg border border-l-4 px-4 py-4 text-sm font-sans shadow-sm transition-all overflow-hidden',
  {
    variants: {
      variant: {
        default: 'bg-card text-foreground border-border border-l-primary/50',
        destructive:
        ...
        info: 'border-blue-200 border-l-blue-500 bg-blue-50 text-blue-900 dark:bg-blue-950/30 dark:border-blue-900/50 dark:border-l-blue-500 dark:text-blue-200',
      },
    },
    defaultVariants: {
      variant: 'default',
...
const AlertTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h5
      ref={ref}
      className={cn(
        'mb-2 flex items-center gap-2 text-lg font-bold leading-none tracking-tight',
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn('font-sans text-sm font-medium leading-relaxed opacity-90', className)}
    {...props}
  />
));
