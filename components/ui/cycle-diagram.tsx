...
export const CycleDiagram: React.FC<CycleDiagramProps> = ({
  ...
  return (
    <div
      className={cn(
        'relative mx-auto aspect-square w-full max-w-md select-none md:aspect-[4/3]',
    ...
    >
      ...
      <div className="mb-8 text-center">
        <h4
          className={cn(
            'font-sans text-xl font-bold tracking-widest',
      ...
      <div className="relative w-full pb-[100%]">
        <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 100" overflow="visible">
          <defs>
            <marker
              id={`arrow-${variant}`}
              markerWidth="6"
        ...
        {data.nodes.map((node, i) => (
          <div
            key={i}
            className={cn(
              'absolute font-serif text-[10px] font-bold leading-tight md:text-xs',
              isPrimary ? 'text-white' : 'text-zinc-500',
              node.align === 'center' && '-translate-x-1/2 text-center',
              node.align === 'left' && 'translate-x-2',
              node.align === 'right' && '-translate-x-2 -translate-x-full text-right'
            )}
        ...
        {data.triangle.map((node, i) => (
          <div
            key={`tri-${i}`}
            className={cn(
              'text-currentColor absolute whitespace-pre font-sans text-[8px] font-medium leading-tight md:text-[9px]',
        ...
        <div
          className={cn(
            'absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 font-sans text-xs font-bold',
            isPrimary ? 'text-white' : 'text-zinc-700 line-through decoration-zinc-500'
          )}
        >
          {data.center}
        </div>
      ...
      <div className="mt-4 text-center">
        <h5 className={cn('font-serif text-lg italic', isPrimary ? 'text-white' : 'text-zinc-500')}>
          {data.subtitle}
        </h5>
      </div>
    </div>
