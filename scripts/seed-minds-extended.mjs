...
const mindExtendedData = {
  ...
  alex_hormozi: {
    ...
    obsessions: [
      {
        name: 'Criação de ofertas irresistíveis',
        intensity: 10,
        notes: 'Grand Slam Offers framework',
      },
      {
        name: 'Value Equation',
        intensity: 9,
        notes: 'Maximizar valor percebido, minimizar fricção',
      },
      {
        name: 'Escalar negócios de serviço',
        intensity: 9,
        notes: 'Gym Launch, Prestige Labs, Acquisition.com',
      },
    ],
    values: [
    ...
    values: [
      {
        ...
        importance: 10,
        notes: 'Conhecer o mercado melhor que ele mesmo',
      },
  ...
  steve_jobs: {
    ...
    obsessions: [
      {
        name: 'Intersecção de tecnologia e humanidades',
        intensity: 10,
        notes: 'Liberal arts + technology',
      },
  ...
  paul_graham: {
    ...
    values: [
      {
        name: 'Fazer algo que as pessoas querem',
        importance: 10,
        notes: 'Make something people want',
      },
  ...
  yuval_harari: {
    ...
    values: [
      ...
      { name: 'Compaixão', importance: 9, notes: 'Reduzir sofrimento' },
      {
        ...
        importance: 10,
        notes: 'Milhares de anos, não décadas',
      },
    ],
  },
  ...
  joao_lozano: {
    ...
    values: [{ name: 'Comprometimento', importance: 8, notes: 'Entregar resultados' }],
  },
  ...
  jose_amorim: {
    ...
    obsessions: [{ name: 'Excelência profissional', intensity: 8, notes: 'Melhoria contínua' }],
    values: [{ name: 'Integridade', importance: 9, notes: 'Fazer o certo' }],
  },
  ...
  adriano_marqui: {
    ...
    values: [
      {
        name: 'Excelência no ensino',
        importance: 10,
        notes: 'Transmitir conhecimento com maestria',
      },
...
async function seedExtendedData() {
  ...
  // Get all minds
  const { data: minds, error: mindsError } = await supabase.from('minds').select('id, slug');
  ...
  for (const [slug, data] of Object.entries(mindExtendedData)) {
    ...
    if (data.proficiencies) {
      for (const prof of data.proficiencies) {
        ...
        const { error } = await supabase.from('mind_proficiencies').upsert(
          {
            mind_id: mindId,
            skill_id: skillId,
            level_10: prof.level,
