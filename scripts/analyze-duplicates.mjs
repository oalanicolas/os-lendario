...
duplicates.forEach(([name, items]) => {
  ...
  items.forEach((m) => {
    ...
    const hasRels =
      (m.proficiencies?.[0]?.count || 0) +
      (m.profiles?.[0]?.count || 0) +
      (m.project_minds?.[0]?.count || 0);
    console.log(
      '  ' +
        m.slug.padEnd(25) +
        ' | bio:' +
...
duplicates.forEach(([name, items]) => {
  ...
  const sorted = items.sort((a, b) => {
    const aScore =
      (a.short_bio ? 10 : 0) + (a.proficiencies?.[0]?.count || 0) + (a.profiles?.[0]?.count || 0);
    const bScore =
      (b.short_bio ? 10 : 0) + (b.proficiencies?.[0]?.count || 0) + (b.profiles?.[0]?.count || 0);
