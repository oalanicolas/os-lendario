...
async function getWikipediaImage(articleTitle, lang = 'en') {
  ...
  return new Promise((resolve, reject) => {
    https
      .get(apiUrl, { headers: { 'User-Agent': 'MindImageDownloader/1.0' } }, (res) => {
        let data = '';
...
async function downloadImage(url, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    protocol
      .get(url, { headers: { 'User-Agent': 'MindImageDownloader/1.0' } }, (response) => {
        // Handle redirects
...
async function processMind(slug, config) {
  ...
  try {
    ...
    if (!imageUrl) {
      console.log(
        `TODO  ${slug} - needs manual search: ${config.search || config.wiki || config.wikiPt || 'no source'}`
      );
